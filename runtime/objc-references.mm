...
class AssociationsManager {
    ...
    //关联对象就存储这个静态的在AssociationsHashMap中，这就相当于把所有对象的关联对象都存储在一个全局的AssociationsHashMap 类型的map中，map的key是对象的指针地址，value是一个AssociationsHashMap，里面存储了关联对象的key和value。
...
// 获取关联对象：通过key从全局的AssociationsHashMap中拿到存储的value值
